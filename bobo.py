...
def render_tool_contract(protocol: dict[str, Any]) -> str:
    ...
    for tool in protocol["tools"]:
        lines.append(
            f"- `{tool['name']}` {tool['args_preview']} : {tool['description']}"
        )
    return "\n".join(lines)
...
def normalize_tool_call(
    ...
    if tool_name == "claim_handoff":
        ...
        normalized_args = {
            "path": require_non_empty_string(args.get("path"), "args.path"),
            "content": require_non_empty_string(args.get("content"), "args.content"),
        }
        ...
        normalized_args = {
            ...
            "replacement_string": require_non_empty_string(
        ...
        normalized_args = {
            "test_command": require_non_empty_string(
